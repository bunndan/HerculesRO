...
BUILDIN(queueopt)
{
	...
	switch (var) {
		case SQO_ONDEATH:
			if (script_hasdata(st, 4))
				safestrncpy(queue->onDeath, script_getstr(st, 4), EVENT_NAME_LENGTH);
			else
				queue->onDeath[0] = '\0';
			break;
		case SQO_ONLOGOUT:
			if (script_hasdata(st, 4))
				safestrncpy(queue->onLogOut, script_getstr(st, 4), EVENT_NAME_LENGTH);
			else
				queue->onLogOut[0] = '\0';
			break;
		case SQO_ONMAPCHANGE:
			if (script_hasdata(st, 4))
				safestrncpy(queue->onMapChange, script_getstr(st, 4), EVENT_NAME_LENGTH);
			else
				queue->onMapChange[0] = '\0';
			break;
		default:
