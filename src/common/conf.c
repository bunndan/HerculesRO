 ...
 *
 * @param setting[in] The setting to read.
 * @param name[in]    The setting name to lookup.
...
int16 config_setting_get_int16(config_setting_t *setting)
{
	...
	if (setting->value.ival > INT16_MAX)
		setting->value.ival = INT16_MAX;
	if (setting->value.ival < INT16_MIN)
		setting->value.ival = INT16_MIN;
 ...
 * @param out_size[in] The size of the output buffer.
 *
 * @retval CONFIG_TRUE  in case of success.
 * @retval CONFIG_FALSE in case of failure.
 */
int config_setting_lookup_mutable_string(config_setting_t *setting, const char *name, char *out, size_t out_size)
{
	const char *str;
 ...
 *
 * @param config[in]   The configuration to read.
 * @param name[in]     The setting name to lookup.
 * @param out[out]     The output buffer.
 * @param out_size[in] The size of the output buffer.
 *
 ...
 * @retval CONFIG_FALSE in case of failure.
 */
int config_lookup_mutable_string(config_t *config, const char *name, char *out, size_t out_size)
{
	const char *str;
